...
class DDPG(BaseAgent):
    ...
    def step(self, state, reward, done):
        ...
        if done:
            ...
            #if(self.total_reward > -700 or self.episode_num>500):
            #if(self.episode_num ==300):
            if(self.episode_num == 300):
            #if(self.episode_num == 150):
               self.store_weights()
            #if(self.episode_num <= 300):
            self.reset_episode_vars()
    ...
    def store_weights(self):
        ...
        actwghtsfile = os.path.join(util.get_param ('out'),"actor_weights_Takeoff_Damped.h5")
        actwghtsfiletwo = os.path.join(util.get_param ('out'),"save_actor_weights_Takeoff_Damped.h5")
        actorweightsjsonfile = os.path.join(util.get_param ('out'),"actor_weights_Takeoff_Damped.json")
        critwghtsfile = os.path.join(util.get_param ('out'),"critic_weights_Takeoff_Damped.h5")
        critwghtsfiletwo = os.path.join(util.get_param ('out'),"save_critic_weights_Takeoff_Damped.h5")
        criticweightsjsonfile = os.path.join(util.get_param ('out'),"critic_weights_Takeoff_Damped.json")
        
        print("Storing actor weight backup in {}".format(actwghtsfiletwo))
        print("Storing critic weight backup in {}".format(critwghtsfiletwo))

        actmodel = self.actor_local.model

        critmodel = self.critic_local.model

        self.saveactorcritic(actmodel,critmodel,actwghtsfile,critwghtsfile,actwghtsfiletwo,critwghtsfiletwo,actorweightsjsonfile,criticweightsjsonfile)

    def saveactorcritic(self,actmodel,critmodel,actfile,critfile,actorweightsfiletwo,criticweightsfiletwo,actorweightsjsonfile,criticweightsjsonfile):
        
        #actmodel.save(actfile)

        #critmodel.save(critfile)

        actmodel.save_weights(actorweightsfiletwo)
        
        critmodel.save_weights(criticweightsfiletwo)

        #with open(actorweightsjsonfile, 'w') as f:
        	#f.write(actmodel.to_json())


        #with open(criticweightsjsonfile, 'w') as f:
        	#f.write(critmodel.to_json())
