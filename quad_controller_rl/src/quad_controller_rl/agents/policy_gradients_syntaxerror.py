...
class DDPG(BaseAgent):
    ...
    def store_weights(self):
        ...
        actor_weights_file = os.path.join(util.get_param ('out'),"actor_weights_takeoff.h5")
        critic_weights_file = os.path.join(util.get_param ('out'),"critic_weights_takeoff.h5")
        print("Storing actor weight backup in {}".format(actor_weights_file));
        print("Storing critic weight backup in {}".format(critic_weights_file));
        #self.model.save_weights(weights_file)
        #self.actor_local.model.save(actor_weights_file)
        #self.critic_local.model.save(critic_weights_file)
        # ACTOR
        #architecture and weights to HDF5
	self.actor_local.model.save(actor_weights_file)
	# architecture to JSON, weights to HDF5
	self.actor_local.model.save_weights(os.path.join(util.get_param ('out'),"save_actor_weights_Takeoff.h5"))
	with open('actor_weights_Takeoff.json', 'w') as f:
	...
	# architecture to JSON, weights to HDF5
	self.critic_local.model.save_weights(os.path.join(util.get_param ('out'),"save_critic_weights_Takeoff.h5"))
	with open('critic_weights_Takeoff.json', 'w') as f:
        	f.write(self.critic_local.model.to_json())
